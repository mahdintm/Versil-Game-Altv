...
alt.on('authDISCORD', handleBeginAuth);
alt.on('discord:rejoin', rejoin)

function rejoin(player) {
    alt.emitClient(player, 'loginweb:Load');
}

function handleBeginAuth(player, user, pass) {
    ...
    alt.emitClient(player, 'discord:Auth', `${url}&state=${playerToken}`, user, pass);
}